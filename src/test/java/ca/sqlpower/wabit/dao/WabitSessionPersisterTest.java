...
public class WabitSessionPersisterTest extends TestCase {
	...
	public void testPersistJDBCWabitDataSource() throws WabitPersistenceException {
		...
		System.out.println("Testing persistObject on JDBC WabitDataSource.");
		wsp.persistObject(workspaceUUID, "WabitDataSource", wdsJdbcUUID, 0);
	...
	public void testPersistOlapWabitDataSource() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		...
		wsp.persistObject(workspaceUUID, "WabitDataSource", wdsOlapUUID, 0);
	...
	public void testPersistQueryCache() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		...
		wsp.persistObject(workspaceUUID, "QueryCache", queryUUID, 0);
	...
	public void testPersistWabitConstantsContainer() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String queryUUID = generateUUID();
		...
		QueryCache qc = workspace.findByUuid(queryUUID, QueryCache.class);
		assertNotNull("QueryCache could not be persisted.", qc);
		assertEquals("QueryCache has an invalid parent.", workspaceUUID,
				qc.getParent().getUUID());

		System.out.println("Testing begin.");
		wsp.begin();

		System.out.println("Testing persistObject on WabitConstantsContainer.");
		wsp.persistObject(queryUUID, "WabitConstantsContainer", constantsUUID,
				offset++);
		...
		assertNotNull("WabitConstantsContainer could not be persisted.", wcc);
		assertEquals("WabitConstantsContainer has an invalid parent.",
				queryUUID, wcc.getParent().getUUID());
	}
	
	public void testPersistWabitConstantItem() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String queryUUID = generateUUID();
		final String constantsUUID = generateUUID();
		...
		wsp.persistObject(constantsUUID, "WabitConstantItem", constantItemUUID,
				offset++);
		...
		assertEquals("WabitConstantItem has an invalid parent.", constantsUUID,
				wci.getParent().getUUID());
		assertEquals("WabitConstantItem has an invalid name property.",
				"current_date", wci.getName());
	}
	
	public void testPersistWabitTableContainer() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String queryUUID = generateUUID();
	...
	public void testPersistWabitColumnItem() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String queryUUID = generateUUID();
		final String tableUUID = generateUUID();
	...
	public void testPersistWabitJoin() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String queryUUID = generateUUID();
		final String tableUUID = generateUUID();
		...
		assertEquals("WabitColumnItem has an invalid parent.", tableUUID, wci2
				.getParent().getUUID());
		assertEquals("WabitColumnItem has an invalid name property.",
		...
		wsp.persistProperty(joinUUID, "LEFT_JOIN_CHANGED", DataType.REFERENCE,
				tableItemUUID);
		wsp.persistProperty(joinUUID, "RIGHT_JOIN_CHANGED", DataType.REFERENCE,
				tableItem2UUID);

		System.out.println("Testing commit.");
		wsp.commit();

		WabitJoin wabitJoin = workspace.findByUuid(joinUUID, WabitJoin.class);
	...
	public void testPersistOlapQuery() throws WabitPersistenceException {
		...
		wsp.persistObject(workspaceUUID, "OlapQuery", olapQueryUUID, 0);
	...
	public void testPersistWabitOlapAxis() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String olapQueryUUID = generateUUID();
	...
	public void testPersistWabitOlapDimension() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String olapQueryUUID = generateUUID();
		final String olapAxisUUID = generateUUID();
	...
	public void testPersistWabitOlapInclusion() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String olapQueryUUID = generateUUID();
		final String olapAxisUUID = generateUUID();
	...
	public void testPersistWabitOlapExclusion() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String olapQueryUUID = generateUUID();
		final String olapAxisUUID = generateUUID();
		...
		assertEquals("WabitOlapExclusion has an invalid operator property.",
				Operator.MEMBER.name(), woe.getOperator().name());
		assertEquals(
				"WabitOlapExclusion has an invalid uniqueMemberName property.",
				"[Geography].[World].[Europe].[Western Europe].[Austria]", woe
						.getUniqueMemberName());
	}
	...
	public void testPersistChart() throws WabitPersistenceException {
		...
		wsp.persistObject(workspaceUUID, "Chart", chartUUID, 0);
	...
	public void testPersistWabitImage() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		...
		wsp.persistObject(workspaceUUID, "WabitImage", imageUUID, 0);
	...
	public void testPersistTemplate() throws WabitPersistenceException {
		...
		wsp.persistObject(workspaceUUID, "Template", templateUUID, 0);
	...
	public void testPersistReport() throws WabitPersistenceException {
		...
		wsp.persistObject(workspaceUUID, "Report", reportUUID, 0);
	...
	public void testPersistPage() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String reportUUID = generateUUID();
		...
		assertEquals("Page has an invalid name property.", "test-page-name",
				page.getName());
		assertEquals("Page has an invalid width property.", 10, page.getWidth());
	...
	public void testPersistContentBox() throws WabitPersistenceException {
		...
		final String reportUUID = generateUUID();
		final String pageUUID = generateUUID();
		final String contentBoxUUID = generateUUID();
		...
		ContentBox contentBox = workspace.findByUuid(
				contentBoxUUID, ContentBox.class);
	...
	public void testPersistChartRenderer() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String chartUUID = generateUUID();
		final String reportUUID = generateUUID();
		...
		wsp.persistObject(contentBoxUUID, "ChartRenderer",
		...
		assertEquals("ChartRenderer has an invalid parent.",
				contentBoxUUID, cRenderer.getParent().getUUID());
		assertEquals("ChartRenderer has an invalid chart property.", chartUUID,
				cRenderer.getChart().getUUID());
	}
	
	public void testPersistCellSetRenderer() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String olapQueryUUID = generateUUID();
		final String reportUUID = generateUUID();
		...
		wsp.persistObject(contentBoxUUID, "CellSetRenderer",
		...
		assertEquals("CellSetRenderer has an invalid parent.",
				contentBoxUUID, csRenderer.getParent()
						.getUUID());
		assertEquals(
	...
	public void testPersistImageRenderer() throws WabitPersistenceException {
		WabitWorkspace workspace = wsp.getWabitSession().getWorkspace();
		final String workspaceUUID = workspace.getUUID();
		final String reportUUID = generateUUID();
		final String pageUUID = generateUUID();
		...
		wsp.persistObject(contentBoxUUID, "ImageRenderer",
		...
		assertEquals("ImageRenderer has an invalid parent.",
				contentBoxUUID, iRenderer.getParent().getUUID());
	}
